...
# 匹配所有的测试结果文件
matching_files = glob.glob('./correct/*.txt')
...
failed_prints = []
print("===== Start Tests =====")
for correct_file in matching_files:
    correct_file_contents = read_file_contents(correct_file)
    
    file = os.path.join('./yours', os.path.basename(correct_file))
    file_contents = read_file_contents(file)
    
    if file_contents is None:
        failed_msg = f"could not open file: {file}"
        pass_all = False
    else:
        # 获取两个文件内容的最小长度
